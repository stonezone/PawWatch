...
**Phase 9 — Stop Bridge & Release Entitlements (2025-11-11 02:35 HST, v1.0.41)**  
- **Done:** Forwarded the existing `pawwatch://stop-tracking` deep link through WCSession so iOS sends an interactive message (with application-context fallback) to the watch; the watch provider now stops the workout and updates the UI via the new `didReceiveRemoteStop` delegate. Added `Config/pawWatchWidgetExtension.entitlements` and wired CODE_SIGN_ENTITLEMENTS only for Release builds of the iOS app + widget extension so App Group/push access stays gated.  
- **Risks:** If WCSession isn’t activated, the stop request relies on a queued application-context update; we still need a push-based guarantee for suspended phone states. Release-only entitlements are configured locally but still require refreshed provisioning profiles on CI/TestFlight.  
